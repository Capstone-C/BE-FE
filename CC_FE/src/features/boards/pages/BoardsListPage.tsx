...
export default function BoardsListPage() {
  ...
  if (isError || !data)
    return <div className="max-w-7xl mx-auto p-8 text-center text-red-600">오류가 발생했습니다.</div>;
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-6 pb-4 border-b border-gray-200 flex justify-between items-end">
            <div>
              ...
              <p className="mt-2 text-md text-gray-500">다양한 식단 정보를 공유하고 소통해보세요.</p>
            </div>
            <Link
              to="/boards/new"
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            {visiblePosts.length > 0 ? (
                        ...
                        return (
                          <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">
                              {postAuthorId ? (
                                <Link
                                  to={`/members/${postAuthorId}`}
                                  className="hover:text-[#4E652F] hover:underline transition-colors"
                                >
                                  {authorName}
                                </Link>
                              ) : (
              ...
              <div className="text-center py-20 text-gray-500">게시글이 존재하지 않습니다.</div>
            )}
          </div>
        </div>
      </div>
